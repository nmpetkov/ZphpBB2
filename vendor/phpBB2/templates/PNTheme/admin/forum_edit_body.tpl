<table width="100%" border="0" cellpadding="3" cellspacing="1" border="0" class="forumline" bgcolor=""#ffffff">
<tr>
<td class="row2" align="center"><span class="maintitle">{L_FORUM_TITLE}</span></td>
</tr><tr>
<td class="row2"><div class="genmed">{L_FORUM_EXPLAIN}</div></td>
</tr></table>
<form action="{S_FORUM_ACTION}" method="post">
<table width="100%" cellpadding="3" cellspacing="1" border="0" class="forumline" align="center">
<tr> 
<th colspan="2">{L_FORUM_SETTINGS}</th>
</tr>
<tr> 
<td class="row1">{L_FORUM_NAME}</td>
<td class="row2" height="25"> 
<input type="text" size="25" name="forumname" value="{FORUM_NAME}" class="post" />
</td>
</tr>
<tr> 
<td class="row1">{L_FORUM_DESCRIPTION}</td>
<td class="row2"> 
<textarea rows="5" style="width: 350px" cols="35" wrap="virtual" name="forumdesc" class="post">{DESCRIPTION}</textarea>
</td>
</tr>
<tr> 
<td class="row1">{L_CATEGORY}</td>
<td class="row2"> 
<select name="c">{S_CAT_LIST}</select>
</td>
</tr>
<tr> 
<td class="row1">{L_FORUM_STATUS}</td>
<td class="row2"> 
<select name="forumstatus">{S_STATUS_LIST}</select>
</td>
</tr>
<!-- Begin PNphpBB2 Module (Topic/Post sort order) 06-10-2003 -->
<tr> 
<td class="row1">{L_TOPIC_SORTING}</td>
<td class="row2">{S_TOPIC_ORDER_SELECT}</td>
</tr>
<tr> 
<td class="row1">{L_POST_SORTING}</td>
<td class="row2">{S_POST_ORDER_SELECT}</td>
</tr>
<!-- End PNphpBB2 Module (Topic/Post sort order) -->
<tr> 
<td class="row1">{L_EXCLUDE_GLOBAL_ANNOUNCE}</td>
<td class="row2" align="left" valign="middle"> 
<input type="checkbox" name="exclude_global_announce" value="1" {S_EXCLUDE_GBOBAL_ANNOUNCE} />
</td>
</tr>
<tr> 
<td class="row1">{L_SUPPORT_FORUM}</td>
<td class="row2" align="left" valign="middle"> 
<input type="checkbox" name="support_forum" value="1" {S_SUPPORT_FORUM} />
</td>
</tr>
<tr> 
<td class="row1">{L_AUTO_PRUNE}</td>
<td class="row2"> 
<table cellspacing="0" cellpadding="1" border="0">
<tr> 
<td align="right" valign="middle">{L_ENABLED}</td>
<td align="left" valign="middle"> 
<input type="checkbox" name="prune_enable" value="1" {S_PRUNE_ENABLED} />
</td>
</tr>
<tr> 
<td align="right" valign="middle">{L_PRUNE_DAYS}</td>
<td align="left" valign="middle">&nbsp; 
<input type="text" name="prune_days" value="{PRUNE_DAYS}" size="5" class="post" />
&nbsp;{L_DAYS}</td>
</tr>
<tr> 
<td align="right" valign="middle">{L_PRUNE_FREQ}</td>
<td align="left" valign="middle">&nbsp; 
<input type="text" name="prune_freq" value="{PRUNE_FREQ}" size="5" class="post" />
&nbsp;{L_DAYS}</td>
</tr>
</table>
</td>
</tr>
<tr> 
<td colspan="2" align="center" class="cat">{S_HIDDEN_FIELDS} 
<input type="submit" name="submit" value="{S_SUBMIT_VALUE}" class="mainoption" />
</td>
</tr>
</table>
</form>
<br clear="all" />
