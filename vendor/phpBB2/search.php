...
//					WHERE username LIKE '" . str_replace("\'", "''", $search_author) . "'";
		    ...
		    $sql = 'SELECT user_id 
			    FROM ' . USERS_TABLE . "
			    WHERE username $sql_where '" . DataUtil::formatForStore(preg_replace('#\*+#', '%', $search_author)). "'";
// End PNphpBB2 Module
...
//					message_die(GENERAL_ERROR, "Couldn't obtain list of matching users (searching for: $search_author)", "", __LINE__, __FILE__, $sql);
					message_die(GENERAL_ERROR, "Couldn't obtain list of matching users (searching for: " . DataUtil::formatForDisplay($search_author) . ")", "", __LINE__, __FILE__, $sql);
// End PNphpBB2 Module
				}
...
//							$where_sql .= " AND u.user_id = p.poster_id AND u.username LIKE '$search_author' ";
							$where_sql .= " AND u.user_id = p.poster_id AND u.username LIKE '" . DataUtil::formatForStore($search_author) . "'";
// End PNphpBB2 Module
						}
...
//				$poster .= ( $searchset[$i]['user_id'] != ANONYMOUS ) ? $searchset[$i]['username'] : ( ( $searchset[$i]['post_username'] != "" ) ? $searchset[$i]['post_username'] : $lang['Guest'] );
				$poster .= ( $searchset[$i]['user_id'] != ANONYMOUS ) ? DataUtil::formatForDisplay($searchset[$i]['username']) : ( ( $searchset[$i]['post_username'] != "" ) ? DataUtil::formatForDisplay($searchset[$i]['post_username']) : $lang['Guest'] );
// End PNphpBB2 Module
...
//				$topic_author .= ( $searchset[$i]['user_id'] != ANONYMOUS ) ? $searchset[$i]['username'] : ( ( $searchset[$i]['post_username'] != '' ) ? $searchset[$i]['post_username'] : $lang['Guest'] );
				$topic_author .= ( $searchset[$i]['user_id'] != ANONYMOUS ) ? DataUtil::formatForDisplay($searchset[$i]['username']) : ( ( $searchset[$i]['post_username'] != '' ) ? DataUtil::formatForDisplay($searchset[$i]['post_username']) : $lang['Guest'] );
// End PNphpBB2 Module
