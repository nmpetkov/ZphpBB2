...
/* ZphpBB2 => */
		'U_GROUPCP' => $board_config['server_name'] . ModUtil::url('ZphpBB2', 'user', 'groupcp', array(POST_GROUPS_URL => $group_id, 'validate' => true)))
/* <= ZphpBB2 */
	);
	$emailer->send();
	$emailer->reset();
...
//					WHERE username = '" . str_replace("\'", "''", $username) . "'";
				$sql = "SELECT user_id, user_email, user_lang, user_level  
					FROM " . USERS_TABLE . " 
					WHERE username = '" . DataUtil::formatForStore($username) . "'";
// End PNphpBB2 Module
				if ( !($result = $db->sql_query($sql)) )
				{
...
/* ZphpBB2 => */
						'U_GROUPCP' => $board_config['server_name'] . ModUtil::url('ZphpBB2', 'user', 'groupcp', array(POST_GROUPS_URL => $group_id)))
/* <= ZphpBB2 */
					);
					$emailer->send();
					$emailer->reset();
...
/*		'MOD_USERNAME' => $username, */
		...
		'MOD_USERNAME' => DataUtil::formatForDisplay($username),
/* End PNphpBB2 Module */
		'MOD_FROM' => $from,
		'MOD_JOINED' => $joined,
		'MOD_POSTS' => $posts,
...
// Begin PNphpBB2 Module
		...
		$s_hidden_fields .= '<input type="hidden" name ="module" value="' . $modinfo['url'] . '">';
