 ...
 *
 *   $Id: viewforum.php 216 2007-07-18 10:17:55Z slam $
 *
 *
 ***************************************************************************/
...
//	$moderators[] = '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=" . $row['user_id']) . '">' . $row['username'] . '</a>';
	$moderators[] = '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=" . $row['user_id']) . '">' . DataUtil::formatForDisplay($row['username']) . '</a>';
// End PNphpBB2 Module
}
...
{ 
	 $sort_method = isset($_GET['sort']) ? $_GET['sort'] : $_POST['sort']; 
}
else
{
...
{
  $sort_order = isset($_GET['order']) ? $_GET['order'] : $_POST['order'];
...
//		$topic_author .= ( $topic_rowset[$i]['user_id'] != ANONYMOUS ) ? $topic_rowset[$i]['username'] : ( ( $topic_rowset[$i]['post_username'] != '' ) ? $topic_rowset[$i]['post_username'] : $lang['Guest'] );
		$topic_author .= ( $topic_rowset[$i]['user_id'] != ANONYMOUS ) ? DataUtil::formatForDisplay($topic_rowset[$i]['username']) : ( ( $topic_rowset[$i]['post_username'] != '' ) ? DataUtil::formatForDisplay($topic_rowset[$i]['post_username']) : $lang['Guest'] );
// End PNphpBB2 Module
