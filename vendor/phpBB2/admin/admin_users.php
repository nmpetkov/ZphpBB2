 ...
 *
 *   $Id: admin_users.php 192 2007-01-20 15:17:44Z kronos $
 *
 *
 ***************************************************************************/
...
//	$module['Users']['Manage'] = $filename;
	$phpbb_module['Users']['Manage'] = $filename;
// End PNphpBB2 Module
...
//				WHERE poster_id = $user_id";
			$sql = "UPDATE " . POSTS_TABLE . "
				SET poster_id = " . DELETED . ", post_username = '" . DataUtil::formatForStore($username) . "' 
				WHERE poster_id = $user_id";
// End PNphpBB2 Module
			if( !$db->sql_query($sql) )
			{
...
//			$message = $lang['User_deleted'] . '<br /><br />' . sprintf($lang['Click_return_useradmin'], '<a href="' . append_sid("admin_users.$phpEx") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . append_sid("index.$phpEx?pane=right") . '">', '</a>');
			$message = $lang['User_deleted'] . '<br /><br />' . sprintf($lang['Click_return_useradmin'], '<a href="' . append_sid("admin_users.$phpEx") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . append_sid("admin.$phpEx?pane=right") . '">', '</a>');
// End PNphpBB2 Module
...
// Begin PNphpBB2 Module
/*
		$username = ( !empty($_POST['username']) ) ? trim(strip_tags(htmlspecialchars($_POST['username']))) : '';
...
//				WHERE user_id = $user_id";
			$sql = "UPDATE " . USERS_TABLE . "
				SET user_sig = '" . str_replace("\'", "''", $signature) . "', user_viewemail = $viewemail, user_attachsig = $attachsig, user_sig_bbcode_uid = '$signature_bbcode_uid', user_allowsmile = $allowsmilies, user_allowhtml = $allowhtml, user_allowavatar = $user_allowavatar, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_allow_pm = $user_allowpm, user_notify_pm = $notifypm, user_popup_pm = $popuppm, user_style = $user_style, user_active = $user_status, user_rank = $user_rank" . $avatar_sql . "
				WHERE user_id = $user_id";
// End PNphpBB2 Module
...
//			$message .= '<br /><br />' . sprintf($lang['Click_return_useradmin'], '<a href="' . append_sid("admin_users.$phpEx") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . append_sid("index.$phpEx?pane=right") . '">', '</a>');
			$message .= '<br /><br />' . sprintf($lang['Click_return_useradmin'], '<a href="' . append_sid("admin_users.$phpEx") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . append_sid("admin.$phpEx?pane=right") . '">', '</a>');
// End PNphpBB2 Module
...
//			$this_userdata = get_userdata($user_id);
			$this_userdata = phpbb_get_userdata($user_id);
// End PNphpBB2 Module
			if( !$this_userdata )
			{
				message_die(GENERAL_MESSAGE, $lang['No_user_id_specified'] );
...
//						"AVATAR_IMAGE" => "../" . $board_config['avatar_gallery_path'] . '/' . $category . '/' . $avatar_images[$category][$i][$j])
					...
					{
			 	 	 	 $avatar_path = $board_config['avatar_gallery_path'];
					}
					$template->assign_block_vars('avatar_row.avatar_column', array(
 						"AVATAR_IMAGE" => "../" . $board_config['avatar_gallery_path'] . '/' . $category . '/' . $avatar_images[$category][$i][$j],
