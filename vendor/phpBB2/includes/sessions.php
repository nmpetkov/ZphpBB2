...
*/
	...
	if (UserUtil::isLoggedIn()) 
	{
      ...
      list($pn_user_pass) = $db->sql_fetchrow($result);
			...
			$sql= "INSERT INTO " . USERS_TABLE . " (user_id, username, user_regdate, user_password,	user_email,	user_icq,	user_website,	user_occ,	user_from, user_interests, user_sig, user_sig_bbcode_uid,	user_avatar, user_avatar_type, user_viewemail, user_aim, user_yim, user_msnm, user_attachsig, user_allowsmile, user_allowhtml, user_allowbbcode, user_allow_viewonline, user_notify, user_notify_pm, user_popup_pm, user_timezone, user_dateformat, user_lang, user_style, user_level, user_allow_pm, user_active, user_actkey) VALUES ($user_id, '" . DataUtil::formatForStore(UserUtil::getVar('uname')) . "', " . time() . ", '" . $pn_user_pass . "', '" . DataUtil::formatForStore(UserUtil::getVar('email')) . "', '" . DataUtil::formatForStore(UserUtil::getVar('user_icq')) . "', '" . DataUtil::formatForStore($user_site_url) . "', '" . DataUtil::formatForStore(UserUtil::getVar('user_occ')) . "', '" . DataUtil::formatForStore(UserUtil::getVar('user_from')) . "', '" . DataUtil::formatForStore(UserUtil::getVar('user_intrest')) . "', '', '', " . $user_avatar . "0, '" . DataUtil::formatForStore(UserUtil::getVar('user_aim')) . "', '" . DataUtil::formatForStore(UserUtil::getVar('user_yim')) . "', '" . DataUtil::formatForStore(UserUtil::getVar('user_msnm')) . "', " . $board_config['allow_sig'] . ", " . $board_config['allow_smilies'] . ", " . $board_config['allow_html'] . ", " . $board_config['allow_bbcode'] . ", 1, 0, 1, 1, " . (UserUtil::getVar('timezone_offset') - 12) . ", '" . $board_config['default_dateformat'] . "', '" . $board_config['default_lang'] . "', " . $board_config['default_style'] . ", $admin_rights, 1, 1, '')";
...
//			$user_avatar = (UserUtil::getVar('pn_user_avatar') != "blank.gif") ? "user_avatar = '" . DataUtil::formatForStore(UserUtil::getVar('pn_user_avatar')) . "', user_avatar_type = 3, " : '';
      ...
      list($pn_user_pass) = $db->sql_fetchrow($result);
 		  ...
 		  $sql = "UPDATE " . USERS_TABLE . " SET " . $user_avatar . "username = '" . DataUtil::formatForStore(UserUtil::getVar('uname')) . "' ,user_password = '" . $pn_user_pass . "', user_email =  '" . DataUtil::formatForStore(UserUtil::getVar('email')) . "', user_icq = '" . DataUtil::formatForStore(UserUtil::getVar('user_icq')) . "', user_website = '" . DataUtil::formatForStore($user_site_url) . "', user_occ = '" . DataUtil::formatForStore(UserUtil::getVar('user_occ')) . "', user_from = '" . DataUtil::formatForStore(UserUtil::getVar('user_from')) . "', user_interests = '" . DataUtil::formatForStore(UserUtil::getVar('user_intrest')) . "', user_aim = '" . DataUtil::formatForStore(UserUtil::getVar('user_aim')) . "', user_yim = '" . DataUtil::formatForStore(UserUtil::getVar('user_yim')) . "', user_msnm = '" . DataUtil::formatForStore(UserUtil::getVar('user_msnm')) . "', user_timezone = " . (UserUtil::getVar('timezone_offset') - 12) . ", user_level = " . $admin_rights . " WHERE user_id = $user_id";
			...
			if ( stripslashes($row['username']) != UserUtil::getVar('uname') )
			{
			 	 $sql = "UPDATE " . GROUPS_TABLE . "	SET group_name = '" . DataUtil::formatForStore(UserUtil::getVar('uname')) . "' WHERE group_name = '" . $row['username'] . "'";
				 if ( !$result = $db->sql_query($sql) )
				 {
...
//
// ZphpBB2 - added $type parameter (else only)
function append_sid($url, $non_html_amp = false, $type = 'user')
{
// Begin PNphpBB2 Module
/*
	global $SID;
...
*/	
	...
	if ($admin_file == "admin")
    {
	  // ZphpBB2 $url = "index.php?module=" . $modinfo['url']  . "&type=admin&func=admin&pnadmin=" . $url;
	  $url = "index.php?module=" . $modinfo['url']  . "&type=user&func=admin&pnadmin=" . $url;
	}
	else
	{
        // always 'user' - admin area to be accessed from admin panel (ZphpBB2)
 	  $url = "index.php?module=" . $modinfo['url']  . "&type=user&func=" . $url;
	}

  if ( !empty($SID) && !eregi('sid=', $url) && !strstr($_SERVER['HTTP_USER_AGENT'] ,'Googlebot') && !strstr($_SERVER['HTTP_USER_AGENT'] ,'slurp@inktomi.com;')) 
	{
