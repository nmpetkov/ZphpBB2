 ...
 *
 *   $Id: functions_post.php 192 2007-01-20 15:17:44Z kronos $
 *
 *
 ***************************************************************************/
...
// Begin PNphpBB2 Module
//	global $board_config, $lang, $db, $phpbb_root_path, $phpEx;
	global $board_config, $lang, $db, $phpbb_root_path, $phpEx, $ModName;
// End PNphpBB2 Module
	global $userdata, $user_ip;
...
/*							'U_STOP_WATCHING_TOPIC' => $server_protocol . $server_name . $server_port . $script_name . '?' . POST_TOPIC_URL . "=$topic_id&unwatch=topic") */
							'U_TOPIC' => $board_config['server_name'] . $board_config['module_url'] . "viewtopic&" . POST_POST_URL . "=$post_id#$post_id",
							'U_STOP_WATCHING_TOPIC' => $board_config['server_name'] . $board_config['module_url'] . "viewtopic&" . POST_TOPIC_URL . "=$topic_id&unwatch=topic")
/* End PNphpBB2 Module */
						);
...
// Begin PNphpBB2 Module
//
// Fill post image templates (or just the variables) with post images
// Either in a window or inline
//
function generate_post_images($mode, $page_id)
{
	...
	if ($mode == 'window')
	{
		...
		$template->set_filenames(array(
			'postimagesbody' => 'posting_images.tpl')
		);
	}
	else
	{
			$template->set_filenames(array(
			'postimagesbody' => 'post_images_body.tpl')
		);
	}
	...
	if ($num_post_images)
	{
		...
		for ($i=0; $i < count($post_images); $i++)
		{
			...
			$template->assign_block_vars('post_images_row.post_images_column', array(
				'POST_IMAGE' => $board_config['path_posticons'] . "/image_" . $post_images[$i] . ".gif", 
				'POST_IMAGE_NUM' => $post_images[$i],
				'POST_IMAGE_CHECKED' => ($msg_icon == $post_images[$i]) ? "checked" : "",
				'POST_IMAGE_NAME' => $post_images[$i])
			);
...
//		}
		...
		$template->assign_vars(array(
			...
			'L_CLOSE_WINDOW' => $lang['Close_window'],
			'POSTICONS_SIZE' => $board_config['posticons_size'],
			'S_POST_IMAGE_COLSPAN' => $s_colspan)
		);
	}
