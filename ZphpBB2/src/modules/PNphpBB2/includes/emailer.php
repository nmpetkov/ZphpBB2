...
/***************************************************************************
    ...
    $Id: emailer.php 192 2007-01-20 15:17:44Z kronos $
...
//		$this->extra_headers = (($this->reply_to != '') ? "Reply-to: $this->reply_to\n" : '') . (($this->from != '') ? "From: $this->from\n" : "From: " . $board_config['board_email'] . "\n") . "Return-Path: " . $board_config['board_email'] . "\nMessage-ID: <" . md5(uniqid(time())) . "@" . $board_config['server_name'] . ">\nMIME-Version: 1.0\nContent-type: text/plain; charset=" . $this->encoding . "\nContent-transfer-encoding: 8bit\nDate: " . date('r', time()) . "\nX-Priority: 3\nX-MSMail-Priority: Normal\nX-Mailer: PHP\nX-MimeOLE: Produced By phpBB2\n" . $this->extra_headers . (($cc != '') ? "Cc: $cc\n" : '')  . (($bcc != '') ? "Bcc: $bcc\n" : ''); 
		...
		$extra_headers .= ($this->from != '') ? "From: $this->from\n" : "From: " . $board_config['sitename'] . " <" . $board_config['board_email'] . ">\n";
		$extra_headers .= "Return-Path: " . $board_config['sitename'] . " <" . $board_config['board_email'] . ">\n";
		$extra_headers .= "Message-ID: <" . md5(uniqid(time())) . "@" . $board_config['server_name'] . ">\n";
		$extra_headers .= "Content-type: text/plain; charset=" . $this->encoding . "\n";
		$extra_headers .= "Content-transfer-encoding: 8bit\n";
		$extra_headers .= "Date: " . date('r', time()) . "\n";
		$extra_headers .= "X-Mailer: PNphpBB2 " . $board_config['version'] ."\n";
		$extra_headers .= ($cc != '') ? "Cc: $cc\n" : '';
		$extra_headers .= ($bcc != '') ? "Bcc: $bcc\n" : '';
...
// Begin PNphpBB2 Module (Attempt to fix email notification problem by mihil) 			
//			$to = ($to == '') ? (($board_config['sendmail_fix']) ? ' ' : 'Undisclosed-recipients:;') : $to;
			$to = ($to == '') ? (($board_config['sendmail_fix'] && !$this->use_smtp) ? ' ' : '') : $to;
// End PNphpBB2 Module (Attempt to fix email notification problem by mihil) 			
