 ...
 *
 *   $Id: admin_post_text_replace.php 192 2007-01-20 15:17:44Z kronos $
 *
 *
 ***************************************************************************/
...
if( isset($_POST['test']) && !empty($_POST['find_text']) && !empty($_POST['replace_with']) )
{
  $find_text = $_POST['find_text'];
	$replace_with = $_POST['replace_with'];
	$sql = "SELECT count(*) FROM " . POSTS_TEXT_TABLE . " WHERE post_text LIKE '%" . DataUtil::formatForStore(htmlspecialchars($_POST['find_text'])) . "%';";
	if ( $result = $db->sql_query($sql) )
	{
		 ...
		 if ($testcount > 0)
		 {
		 		$sql = "SELECT post_id, post_text FROM " . POSTS_TEXT_TABLE . " WHERE post_text LIKE '%" . DataUtil::formatForStore(htmlspecialchars($_POST['find_text'])) . "%';";
		 		$result = $db->sql_query($sql);
...
if ( !empty($_POST['find_text']) && !empty($_POST['replace_with']) )
{
  $find_text = $_POST['find_text'];
	$replace_with = $_POST['replace_with'];
}
