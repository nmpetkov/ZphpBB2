...
//				VALUES ('" . $next_id . "', '" . str_replace("\'", "''", $_POST['forumname']) . "', " . intval($_POST[POST_CAT_URL]) . ", '" . str_replace("\'", "''", $_POST['forumdesc']) . "', $next_order, " . intval($_POST['forumstatus']) . ", " . intval($_POST['prune_enable']) . $value_sql . ")";
			$sql = "INSERT INTO " . FORUMS_TABLE . " (forum_id, forum_name, cat_id, forum_desc, forum_order, forum_status, prune_enable, topic_sort_order, post_sort_order, exclude_global_announce, support_forum" . $field_sql . ")
				VALUES ('" . $next_id . "', '" . str_replace("\'", "''", $_POST['forumname']) . "', " . intval($_POST[POST_CAT_URL]) . ", '" . str_replace("\'", "''", $_POST['forumdesc']) . "', $next_order, " . intval($_POST['forumstatus']) . ", " . intval($_POST['prune_enable']) . ", " . intval($_POST['topic_sort_order']) . ", " . intval($_POST['post_sort_order']) . ", " . intval($_POST['exclude_global_announce']) . ", " . intval($_POST['support_forum']) . $value_sql . ")";
...
// Begin PNphpBB2 Module
			if( isset($_POST['exclude_global_announce']))
			{
				if( $_POST['exclude_global_announce'] != 1 )
				{
					$_POST['exclude_global_announce'] = 0;
				}
			}

			if( isset($_POST['support_forum']))
			{
				if( $_POST['support_forum'] != 1 )
				{
					$_POST['support_forum'] = 0;
				}
			}
// End PNphpBB2 Module
...
//				WHERE forum_id = " . intval($_POST[POST_FORUM_URL]);
			$sql = "UPDATE " . FORUMS_TABLE . "
				SET forum_name = '" . str_replace("\'", "''", $_POST['forumname']) . "', cat_id = " . intval($_POST[POST_CAT_URL]) . ", forum_desc = '" . str_replace("\'", "''", $_POST['forumdesc']) . "', forum_status = " . intval($_POST['forumstatus']) . ", prune_enable = " . intval($_POST['prune_enable']) . ", post_sort_order = " . intval($_POST['post_sort_order']) . ", exclude_global_announce = " . intval($_POST['exclude_global_announce']) . ", support_forum = " . intval($_POST['support_forum']) . ", topic_sort_order = " . intval($_POST['topic_sort_order']) . "
				WHERE forum_id = " . intval($_POST[POST_FORUM_URL]);
// End PNphpBB2 Module (Topic/Post sort order)
			if( !$result = $db->sql_query($sql) )
			{
...
//				WHERE cat_id = " . intval($_POST[POST_CAT_URL]);
			$sql = "UPDATE " . CATEGORIES_TABLE . "
				SET cat_title = '" . htmlspecialchars(str_replace("\'", "''", $_POST['cat_title'])) . "', cat_main = ".intval($_POST['cat_main']). ", cat_desc = '" . str_replace("\'", "''", $_POST['cat_desc']) . "' ".
				" WHERE cat_id = " . intval($_POST[POST_CAT_URL]);
// End PNphpBB2 Categories Hierarchie Mod
			if( !$result = $db->sql_query($sql) )
			{
