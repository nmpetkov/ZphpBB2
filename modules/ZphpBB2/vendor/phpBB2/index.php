 ...
 *
 *   $Id: index.php 217 2007-07-18 10:22:10Z slam $
 *
 *
 ***************************************************************************/
...
// ZphpBB2 - global to be possible to include from functions (enhance_cat, display_index_cat)
...
global $total_categories, $total_forums, $forum_moderators, $max_depth;
...
//		$forum_moderators[$row['forum_id']][] = '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=" . $row['user_id']) . '">' . $row['username'] . '</a>';
		$forum_moderators[$row['forum_id']][] = '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=" . $row['user_id']) . '">' . DataUtil::formatForDisplay($row['username']) . '</a>';
// End PNphpBB2 Module
	}
	$db->sql_freeresult($result);
...
/*		'NEWEST_USER' => sprintf($lang['Newest_user'], '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=$newest_uid") . '">', $newest_user, '</a>'),  */
		'NEWEST_USER' => sprintf($lang['Newest_user'], '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=$newest_uid") . '">', DataUtil::formatForDisplay($newest_user), '</a>'), 
/* End PNphpBB2 Module */
			...
			if( $userdata['session_logged_in'] )
			{
				if( $topicrow[$i]['post_time'] > $userdata['user_lastvisit'] ) 
				{
					if( !empty($tracking_topics) || !empty($tracking_forums) || isset($_COOKIE[$board_config['cookie_name'] . '_f_all']) )
					{
						...
						{
							if( $_COOKIE[$board_config['cookie_name'] . '_f_all'] >= $topicrow[$i]['post_time'] ) $unread_topics = false;
						}
