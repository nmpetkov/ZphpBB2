...
*
*     $Id: functions_search.php 192 2007-01-20 15:17:44Z kronos $
*
****************************************************************************/
...
//			ORDER BY username";
		$sql = "SELECT username 
			FROM " . USERS_TABLE . " 
			WHERE username LIKE '" . pnVarPrepForStore($username_search) . "' AND user_id <> " . ANONYMOUS . "
			ORDER BY username";
// End PNphpBB2 Module
		if ( !($result = $db->sql_query($sql)) )
		{
...
//				$username_list .= '<option value="' . $row['username'] . '">' . $row['username'] . '</option>';
				$username_list .= '<option value="' . $row['username'] . '">' . pnVarPrepForDisplay($row['username']) . '</option>';
// End PNphpBB2 Module
			}
			while ( $row = $db->sql_fetchrow($result) );
		}
...
/*		'USERNAME' => ( !empty($search_match) ) ? phpbb_clean_username($search_match) : '', */ 
		'USERNAME' => ( !empty($search_match) ) ? pnVarPrepForDisplay($search_match) : '', 
/* End PNphpBB2 Module */
