...
mysql_select_db($DBname) or die("Could not select database");
  ...
  if ($result && mysql_numrows($result) > 0)
    while ($row = mysql_fetch_array($result)) {
	  ...
	  /* $query = "INSERT INTO ".$prefix."_dizkus_posts_text (post_id, post_text) VALUES (".$post_id.", '".addslashes($post_text)."')"; */
	  $query = "UPDATE ".$prefix."_dizkus_posts SET post_text='".addslashes($post_text)."' WHERE post_id=".$post_id;
      $res = mysql_query($query);
      if (!$res) {
         echo "Could not successfully run query ($query) from DB: " . mysql_error();
         exit;
  ...
  if ($result && mysql_numrows($result) > 0)
    while ($row = mysql_fetch_array($result)) {
      ...
      /* $query = "INSERT INTO ".$prefix."_dizkus_topic_subscription (topic_id, user_id, forum_id)
                VALUES (".$topic_id.", ".$user_id.", ".$forum_id.")"; */
      $query = "INSERT INTO ".$prefix."_dizkus_topic_subscription (topic_id, user_id)
